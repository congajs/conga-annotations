...
AttributeParser.prototype = {
	...
	parse: function(string){
		...
		while (isContinue){
			...
			// check for object or array close
			var isArray = part.trim()[part.trim().length - 1] === ']' && !part.trim().match(/(.*)=\[/);
			var isObject = part.trim()[part.trim().length - 1] === '}' && !part.trim().match(/(.*)=\{/);	
			...
			if (isArray || isObject){
				...
				while (!found){
					...
					if ((isArray && part.trim().match(/(.*)=\[/)) || (isObject && part.trim().match(/(.*)=\{/))){
						found = true;
					}	
				}
